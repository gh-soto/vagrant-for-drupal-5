$Id: README.txt,v 1.1.2.2 2008/11/10 10:12:30 francoud Exp $

          JOIN_ROLE_WITH_PASSWORD - README

--------------------------------------------------------------------
NAME:       JOIN_ROLE_WITH_PASSWORD
...
can then manage roles' passwords accessing: admin/user/role_password
Here you can:
 - add a password to a role: when u add a password to a role, users with the "May join new role"
 ...
 - change password to a role: if you previously defined a password for a role, you can also
   change it  (admin/user/role_password/changepwd)
...
Site administrator should give "May join new role" permission to selected user's role, but also to
all authenticated users. 

Users with the 'May join new role' permission can Join or Leave a role. They will find a
...
When users want to join or leave a a role, he receive a list of roles from that table, if they also
exists in the "role" table.

When admin remove a password to a role, the entry is removed from roles_passwords table. No
modification is never done to the "role" table.
