...
$(document).ready(function() {
  ...
  var l = $('<a id="relatedlinks-clear" href="#">Clear</a>')
    .appendTo(fields)
    .click(function() {
      current = -1;
      $('#field-relatedlinks-url', fields).val('');
  ...
  function links_preview() {
    ...
    table = $('<table><tbody><th>ID</th><th>Link</th><th colspan="4">Operations</th></tbody></table>').appendTo(preview);
    ...
    for (var i = 0; i < links.length; i++) {
      ...
      // Add edit button.
      actions = '<td class="relatedlinks-action relatedlinks-edit"><a title="Edit this link.">Edit</a></td>';
      // Add delete button.
      actions += '<td class="relatedlinks-action relatedlinks-delete"><a title="Delete this link.">Delete</a></td>';

      // If available, add the up button.
      if (i > 0) {
        actions += '<td class="relatedlinks-action relatedlinks-up"><a title="Move this link one row up.">Up</a></td>';
      }
      else {
      ...
      if (i < links.length - 1) {
        actions += '<td class="relatedlinks-action relatedlinks-down"><a title="Move this link one row down.">Down</a></td>';
      }
      else {
  ...
  function url_sanitise(url) {
    ...
    if (!matches) {
      url = Drupal.settings.relatedlinks.base_path + (Drupal.settings.relatedlinks.clean_urls ? url : '?q=' + url);
    } 
